...
public class WompiController {
     ...
     * Requiere JWT de ESTUDIANTE. La cédula se toma del token, no del body.
     *
    ...
    @PostMapping("/crear")
    public ResponseEntity<?> crearPago(@RequestBody Map<String, Object> body, Authentication auth) {
        ResolvedPrincipal p = principalResolver.resolve(auth);
        if (p == null) return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
    ...
    public ResponseEntity<?> crearPagoCertificado(@RequestBody Map<String, Object> body, Authentication auth) {
        ResolvedPrincipal p = principalResolver.resolve(auth);
        if (p == null) return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                .body(Map.of("error", "No autenticado"));
        try {
            Long certId = Long.valueOf(body.get("certificadoId").toString());
            return ResponseEntity.ok(wompiService.crearPagoCertificado(certId, p.cedula()));
